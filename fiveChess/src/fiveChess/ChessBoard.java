...
public class ChessBoard extends JPanel implements MouseListener{
	...
	Point[] chessList = new Point[ (ROWS+1)*(COLS+1) ]; //初始每个数组个数为null
	boolean isBlack=true; //默认黑先
	boolean gameOver=false; //游戏是否结束
	int chessCount; //当前棋盘棋子个数
...
public void paintComponent(Graphics g) {
	...
	int imgHeigh=img.getHeight(this);//获得图片宽度和高度
	int FWidth=getWidth();
	int FHeight=getHeight();//获得窗口的宽度和高度
	int x=(FWidth-imgWidth)/2;
	int y=(FHeight-imgHeigh)/2;
	...
	for(int i=0; i<chessCount;i++) {
		...
		if(colortemp==Color.black) {
			...
			RadialGradientPaint paint = new RadialGradientPaint(xPos-Point.DIAMETER/2+25, yPos-Point.DIAMETER/2+10, 70
